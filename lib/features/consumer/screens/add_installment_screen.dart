...
class _InstallmentFormViewState extends State<_InstallmentFormView> {
    ...
    if (_formKey.currentState!.validate() && _selectedDate != null) {
      try {
        ...
        await _controller.saveInstallment(
          storeName: _storeNameController.text.isNotEmpty ? _storeNameController.text : (_selectedProvider ?? 'Unknown Store'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Installment added successfully!'), backgroundColor: Colors.green),
          );
  ...
  void _nextStep() {
    if (_currentStep == 0) {
      // Validate Step 1
      if (_selectedProvider == null || _itemDescController.text.isEmpty || _storeNameController.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please fill all required fields in Step 1.'), backgroundColor: Colors.red),
        );
        return;
      }
    } else if (_currentStep == 1) {
      // Validate Step 2
      if (_totalAmountController.text.isEmpty || _installmentsController.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please fill all required fields in Step 2.'), backgroundColor: Colors.red),
        );
        return;
      }
    }
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              Text('${(progress * 100).toInt()}% 🔥', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.orange)),
            ],
          ),
          ...
          Row(
            ...
            children: [
              ...
              _buildNode(label: 'Item', isCompleted: _currentStep > 0, isActive: _currentStep == 0),
              _buildNode(label: 'Finance', isCompleted: _currentStep > 1, isActive: _currentStep == 1),
              _buildNode(label: 'Finalize', isCompleted: false, isActive: _currentStep == 2),
            ],
          )
        ],
      ),
  ...
  Widget _buildNode({required String label, required bool isCompleted, required bool isActive}) {
    Widget icon;
    if (isCompleted) {
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: isCompleted || isActive ? const Color(0xFF99AFD7) : Colors.grey[300],
            shape: BoxShape.circle,
            border: Border.all(color: isActive ? const Color(0xFF99AFD7) : Colors.transparent, width: 2),
          ),
          alignment: Alignment.center,
          child: icon,
        ),
        const SizedBox(height: 4),
        Text(label, style: TextStyle(fontSize: 12, fontWeight: isActive ? FontWeight.bold : FontWeight.normal, color: isActive ? const Color(0xFF99AFD7) : Colors.grey)),
      ],
    );
  }
  ...
  Widget _buildStep1() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Step 1: Item & Provider', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        _buildLabel('Store / Merchant Name'),
        _buildTextField(_storeNameController, 'e.g., B.TECH', suffixIcon: Icons.store),
        const SizedBox(height: 16),
        ...
        GestureDetector(
          ...
          child: AbsorbPointer(
            child: TextFormField(
              ...
              validator: (val) => _selectedProvider == null ? 'Required field' : null,
            ),
          ),
        ),
        const SizedBox(height: 16),
  ...
  Widget _buildStep2() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Step 2: Financials & Presets', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
        ...
        Wrap(
          ...
          runSpacing: 8.0,
          children: [
            ..._monthPresets.map((months) => ChoiceChip(
              label: Text('$months Months'),
              selected: _selectedPresetMonths == months && !_showCustomMonths,
              selectedColor: const Color(0xFF99AFD7),
              labelStyle: TextStyle(color: _selectedPresetMonths == months && !_showCustomMonths ? Colors.white : Colors.black),
              onSelected: (selected) {
                if (selected) {
                  setState(() {
            ...
            ChoiceChip(
              ...
              selectedColor: const Color(0xFF99AFD7),
              labelStyle: TextStyle(color: _showCustomMonths ? Colors.white : Colors.black),
              onSelected: (selected) {
                if (selected) {
                  setState(() {
        ...
        if (_showCustomMonths) ...[
          const SizedBox(height: 16),
          _buildTextField(_installmentsController, 'e.g., 18', keyboardType: TextInputType.number),
        ],
        const SizedBox(height: 16),
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text('EGP ${_calculatedMonthly.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
                ],
              ),
            ],
          ),
  ...
  Widget _buildStep3() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Step 3: Details & Smart Dates', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        if (widget.isLongTerm) ...[
        ...
        DropdownButtonFormField<String>(
          ...
          decoration: _dropdownDecoration(),
          items: currentCategories.map((cat) => DropdownMenuItem(value: cat, child: Text(cat))).toList(),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(_currentStep == 2 ? 'Add Installment' : 'Next', style: const TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
