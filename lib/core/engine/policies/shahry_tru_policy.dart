...
/// Shahry / TRU late fee policy (identical rules, shared implementation):
/// - 5-day grace period
/// - Pre-acceleration: 15% of missed principal per missed month
/// - Acceleration at 2+ missed months: 15% of entire remaining principal
class ShahryTruPolicy implements LateFeePolicy {
  const ShahryTruPolicy();
  ...
  PenaltyResult calculatePenalty(
    ...
    if (daysLate > 5) {
      for (int i = 1; i <= uncappedMissedMonths; i++) {
        ...
        totalLateFee += (actualMonths * inst.monthlyPayment) * 0.15;
      }
    }
